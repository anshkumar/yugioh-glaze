...
function c1074.filter(c)
	return c:IsFaceup() and c:IsCanTurnSet()
end
function c1074.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c1074.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(Card.IsFacedown,tp,LOCATION_MZONE,0,1,nil)
	and	Duel.IsExistingTarget(c1074.filter,tp,0,LOCATION_MZONE,1,nil) end
...
function c1074.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.ChangePosition(g,POS_FACEUP_DEFENCE)>0 then
		local ct=Duel.GetMatchingGroup(c1074.filter,tp,0,LOCATION_MZONE,nil)
