...
function c4060.initial_effect(c)
	...
	e1:SetTarget(c4060.target)
	e1:SetOperation(c4060.operation)
	c:RegisterEffect(e1)
end
c4060[0]=0
c4060[1]=0
function c4060.target(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c4060.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsAttackPos() and tc:IsRelateToEffect(e) then
		...
		if tc:IsPosition(POS_FACEUP_DEFENCE) then
			...
			e1:SetValue(c4060.val)
			e1:SetReset(RESET_PHASE+PHASE_END,1)
			Duel.RegisterEffect(e1,tp)
		end
	end
end
function c4060.val(e,re,dam,r,rp,rc)
	if c4060[e:GetOwnerPlayer()]==1 or bit.band(r,REASON_EFFECT)~=0 then
		return dam/2
	else return dam end
end
