...
function c1018.initial_effect(c)
        ...
        e1:SetDescription(aux.Stringid(1018,0))
        ...
        e3:SetDescription(aux.Stringid(1018,2))
        ...
        e3:SetCondition(c1018.spcon)
        e3:SetCost(c1018.spcost)
        e3:SetTarget(c1018.sptg)
        e3:SetOperation(c1018.spop)
        c:RegisterEffect(e3)
end
...
function c1018.operation(e,tp,eg,ep,ev,re,r,rp)
        ...
        local ac=1
        if ct>1 then
                Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(1018,1))
...
function c1018.spcon(e,tp,eg,ep,ev,re,r,rp)
...
function c1018.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
        if chk==0 then return Duel.GetFlagEffect(tp,1018)==0 end
        Duel.RegisterFlagEffect(tp,1018,RESET_PHASE+PHASE_END,0,1)
end
...
function c1018.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
        if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
                and Duel.IsExistingTarget(c1018.filter,tp,LOCATION_DECK,0,1,nil,e,tp) end
        Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,g,1,0,0)
end
function c1018.spop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<=0 then return end
    Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
    local g=Duel.SelectMatchingCard(tp,c2295440.filter,tp,LOCATION_DECK,0,1,1,nil,e,tp)
