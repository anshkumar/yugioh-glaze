...
function c1015.initial_effect(c)
	...
	e3:SetCondition(c1015.condtion)
	e3:SetTarget(c1015.tg)
	e3:SetOperation(c1015.activate)
	c:RegisterEffect(e3)
end
...
function c1015.condtion(e,tp,eg,ep,ev,re,r,rp)
...
function c1015.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and c1015.filter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c1015.filter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,c1015.filter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
